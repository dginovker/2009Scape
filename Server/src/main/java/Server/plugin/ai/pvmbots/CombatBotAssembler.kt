...
class CombatBotAssembler {
    ...
    fun RangeAdventurer(tier: Tier, location: Location): CombatBot{
        ...
        bot.equipment.add(Item(Items.BRONZE_BOLTS,100000),13,false,false)
        bot.equipment.refresh()
        return bot
    }
    ...
    fun gearRangedBot(bot: AIPlayer, crossbow: Boolean? = false) {
        ...
        equipHighest(bot, NGLOVES)
        equipHighest(bot, NRBOOTS)
        if(crossbow == true) { equipHighest(bot,CROSSBOWS); equipHighest(bot,MELEE_SHIELD); bot.equipment.add(Item(Items.BRONZE_BOLTS,Integer.MAX_VALUE),13,false,false) }
        else {equipHighest(bot, BOWS); bot.equipment.add(Item(Items.BRONZE_ARROW,Integer.MAX_VALUE),13,false,false) }
        bot.equipment.refresh()
    }
    ...
    fun gearPCnRangedBot(bot: AIPlayer, crossbow: Boolean? = false, vararg skills: Int) {
        ...
        bot.equipment.refresh()
        if(crossbow == true) { equipHighest(bot,PCCROSSBOWS); equipHighest(bot,MELEE_SHIELD); bot.equipment.add(Item(Items.BRONZE_BOLTS,Integer.MAX_VALUE),13,false,false) }
        else {equipHighest(bot, PCBOWS); bot.equipment.add(Item(Items.BRONZE_ARROW,Integer.MAX_VALUE),13,false,false) }
